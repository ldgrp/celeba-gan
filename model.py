...
def build_generator(config: Config) -> models.Model:
    ...
    model = models.Sequential([
        ...
        layers.UpSampling2D(),
        layers.Conv2D(256, kernel_size=kernel_size, use_bias=False, 
...
def build_discriminator(config: Config) -> models.Model:
    ...
    model = models.Sequential([
        ...
        layers.BatchNormalization(momentum=momentum, epsilon=1e-5),
        layers.LeakyReLU(alpha=alpha),
        layers.Dropout(dropout),

        layers.Conv2D(128, kernel_size=kernel_size, strides=2, 
            padding="same", kernel_initializer=normal),
        layers.BatchNormalization(momentum=momentum, epsilon=1e-5),
        layers.LeakyReLU(alpha=alpha),
        layers.Dropout(dropout),

        layers.Conv2D(256, kernel_size=kernel_size, strides=2, 
            padding="same", kernel_initializer=normal),
        layers.BatchNormalization(momentum=momentum, epsilon=1e-5),
        layers.LeakyReLU(alpha=alpha),
        layers.Dropout(dropout),

        layers.Conv2D(512, kernel_size=kernel_size, strides=1, 
            padding="same", kernel_initializer=normal),
        layers.BatchNormalization(momentum=momentum, epsilon=1e-5),
        layers.LeakyReLU(alpha=alpha),
        layers.Dropout(dropout),
